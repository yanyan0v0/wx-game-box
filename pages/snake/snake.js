Page({
  ...
  init() {
    ...
    this.initGround();
    let snake = this.initSnake(this.direction);
  ...
  renderSnake(snake) {
    let snakePositions = snake.map(item => item.position.join());
    let snakeHeader = snake.find(item => item.isHeader);
    this.groundList.forEach(row => {
      row.forEach(col => {
        let snakeIndex = snakePositions.indexOf(col.position.join());
  ...
  initFruit(snake) {
    // 随机生成果实坐标
    let fruit = [
      ~~(this.groundSize[0] * Math.random()),
      ~~(this.groundSize[1] * Math.random()),
    ];
    // 如果与果实坐标位置与蛇重合则重新生成
  ...
  changeSnakeByDirection(direction) {
    ...
    // 判断蛇的头部位置
    let headerIndex = snake.findIndex(item => item.isHeader);
    let newPosition = [...snake[headerIndex].position];
  ...
  checkFruit(newPosition, snake) {
    let isTouch = newPosition.join() === this.fruit.join();
    if (isTouch) {
      this.addScore();
      this.initFruit(snake);
    }

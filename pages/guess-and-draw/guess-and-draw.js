...
Page({
  ...
  touchStart(event) {
    this.step++;
    // 设置原点坐标
    this.drawOrigin = [event.touches[0].x - this.canvasRect.left, event.touches[0].y - this.canvasRect.top];
  ...
  touchMove(event) {
    if (this.startDraw) {
      ...
      const curPoint = [event.touches[0].x - this.canvasRect.left, event.touches[0].y - this.canvasRect.top];
